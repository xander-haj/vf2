...
import { BEDROCK_MAX_HEIGHT, DEEPSLATE_MAX_HEIGHT } from "../game/game-config";
...
function toDepthVariant(normal: BlockId, deepslate: BlockId, y: number): BlockId {
  return y <= DEEPSLATE_MAX_HEIGHT ? deepslate : normal;
}
...
export class TerrainGeology {
  ...
  public getBlock(worldX: number, y: number, worldZ: number, biome: TerrainBiome): BlockId {
    ...
    return rock ?? (y <= DEEPSLATE_MAX_HEIGHT ? BlockId.Deepslate : BlockId.Stone);
  }
  ...
  private getOre(worldX: number, y: number, worldZ: number, biome: TerrainBiome): BlockId | null {
    if (y <= 12 && this.isVein(worldX, y, worldZ, 1409, 0.965)) {
      return toDepthVariant(BlockId.DiamondOre, BlockId.DeepslateDiamondOre, y);
    }
    if (y <= 16 && this.isVein(worldX, y, worldZ, 1423, 0.935)) {
      return toDepthVariant(BlockId.RedstoneOre, BlockId.DeepslateRedstoneOre, y);
    }
    if (y >= 4 && y <= 24 && this.isVein(worldX, y, worldZ, 1433, 0.955)) {
      return toDepthVariant(BlockId.LapisOre, BlockId.DeepslateLapisOre, y);
    }
    if (y <= 20 && this.isVein(worldX, y, worldZ, 1447, 0.95)) {
      return toDepthVariant(BlockId.GoldOre, BlockId.DeepslateGoldOre, y);
    }
    if (
      ...
      return toDepthVariant(BlockId.EmeraldOre, BlockId.DeepslateEmeraldOre, y);
    }
    if (y >= 3 && y <= 38 && this.isVein(worldX, y, worldZ, 1459, 0.89)) {
      return toDepthVariant(BlockId.IronOre, BlockId.DeepslateIronOre, y);
    }
    if (y >= 4 && y <= 40 && this.isVein(worldX, y, worldZ, 1471, 0.905)) {
      return toDepthVariant(BlockId.CopperOre, BlockId.DeepslateCopperOre, y);
    }
    if (y >= 8 && this.isVein(worldX, y, worldZ, 1481, 0.88)) {
      return toDepthVariant(BlockId.CoalOre, BlockId.DeepslateCoalOre, y);
    }
    return null;
  }
  ...
  private getRockVariant(worldX: number, y: number, worldZ: number): BlockId | null {
    ...
    if (y > DEEPSLATE_MAX_HEIGHT && this.noise.value2d(sampleX, sampleZ) > 0.62) {
      return BlockId.Granite;
    }
    if (y > DEEPSLATE_MAX_HEIGHT && this.noise.value2d(sampleX + 310, sampleZ - 210) > 0.64) {
      return BlockId.Diorite;
    }
    if (y > DEEPSLATE_MAX_HEIGHT && this.noise.value2d(sampleX - 170, sampleZ + 270) > 0.63) {
